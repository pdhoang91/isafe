...
//     const [error, setError] = useState(null);
//     const [result, setResult] = useState(null);
//     const [autoDetect, setAutoDetect] = useState(true); // State để bật/tắt tự động nhận diện
...
//         };
//     }, [autoDetect]); // Thêm autoDetect vào dependency array
...
//                 Face Auto Scan
//             </Typography>
//             <IconButton onClick={handleToggleAutoDetect} color="primary">
//                 {autoDetect ? <Pause /> : <PlayArrow />}
//             </IconButton>
//             <div style={{ position: 'relative', width: '400px' }}>
//                 <Webcam
//                     ref={webcamRef}
...
import { nets, detectAllFaces, TinyFaceDetectorOptions, draw } from 'face-api.js';
import { Typography, CircularProgress, Alert, Box, IconButton } from '@mui/material';
import { PlayArrow, Pause, Videocam, VideocamOff } from '@mui/icons-material';
import { verifyFace } from '../services/api';
...
function FaceAutoScanBox() {
    ...
    const [error, setError] = useState(null);
    const [result, setResult] = useState(null);
    const [autoDetect, setAutoDetect] = useState(true); // Bật/tắt chế độ tự động nhận diện
    const [webcamActive, setWebcamActive] = useState(true); // Bật/tắt webcam
    ...
    }, [autoDetect, webcamActive]); // Theo dõi cả `autoDetect` và `webcamActive`
    ...
    return (
        <div>
            ...
            <Box display="flex" gap={1}>
                ...
                <IconButton onClick={handleToggleWebcam} color="primary">
                    {webcamActive ? <Videocam /> : <VideocamOff />}
                </IconButton>
            </Box>
            <div style={{ position: 'relative', width: '400px' }}>
                {webcamActive && (
